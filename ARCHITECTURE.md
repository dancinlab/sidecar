...
| `sync` | 셸 스크립트 래퍼 |
| `pool` | 호스트 로스터 + 원격 실행. `shared:false` 제한 호스트는 `allow` 프로젝트 컨텍스트(cwd 경로 세그먼트) 밖에서 `on`/`status` 차단 — 사적/연구 전용 머신이 공용 컴퓨트로 새는 걸 막음 (env `HARNESS_POOL_ALLOW=<name>` = 의도적 override). 원격 실행은 `execArgs("ssh", [...SSH_ARGS, target, cmd])` 로 ssh 를 **직접 spawn**(로컬 셸 경유 X) — `cmd` 가 단일 argv 라 로컬 `$`/`$(...)` 전개가 안 일어나고 원격 셸에서만 전개됨 |
