...
- **bypass 0.1.0 — new skill** — kills the "next user action:" punt. When the agent is about to emit a block of bash commands as user-action and the agent itself can run them (no interactive human input · no unauthorized destructive ops · no external visible messages), just execute. Triggers on natural language ("bypass" · "그냥 해" · "그냥 진행해" · "do it yourself" · "just run it") OR the agent's own punt-pattern self-check.
