...
- Write/Edit gate + Bash 쓰기채널 둘 다 `.hpp`/`.hh` 커버.
- **literal 매칭** — `.hh`≠`.hexa` · `.hpp`≠`.h`. `.inl`/`.ipp`/`.tcc`는 비대상(미요청).
- 버전 lockstep(g22): plugin.json·marketplace.json 0.6.0→**0.7.0** · CHANGELOG.
...
- Write/Edit gate + Bash 쓰기채널(`> X.s`·heredoc·tee·dd·cp/mv) 둘 다 신규 3 확장자 커버.
- **literal 매칭** — `.c`≠`.cc` · `.s`≠`.css`/`.js`/`.ts` · `.o`≠`.go`/`.io`. `.cpp`/`.cxx`/`.h`는 비대상(미요청).
- 비쓰기(`gcc -c X.c -o X.o`·`cat X.c`)는 통과 (컴파일러 `-o` 출력은 미커버 채널).
- 버전 lockstep(g22): plugin.json·marketplace.json 0.4.0→**0.5.0** · CHANGELOG.
...
- **fix** — `json_parse(pjraw)` 후 `has_key(pjm, "mcpServers")` 로 **파싱된 top-level 키**를 검사. 설명 산문에 `mcpServers` 가 등장해도 이제 무탐. 파일 내 기존 패턴(marketplace/pj 파싱)과 동일 스타일.
- plugin.json + marketplace + README 0.7.0→0.7.1 lockstep (g22). hexa 4줄 교체.
...
## 2026-05-28 — step-by-step 0.7.0: ship+QA 직후 자동 HANDOFF.md 9-section 인계 doc

📋 sbs 0.6.0(자동 QA) 다음 단계 — handoff agent 가 ship + 4축 QA 직후 인계 문서를 자동 생성. (plugin.json 은 0.7.0 로 이미 shipping 됐는데 marketplace/README/CHANGELOG lockstep 이 누락돼 있어 sidecar-lint 가 버전 드리프트로 잡음 → 이번에 보정.)

- **자동 HANDOFF.md 9-section** — trigger 키워드 또는 PR≥3/LOC≥500 휴리스틱 시, `domains/HANDOFF.md`(또는 레포 루트) 에 9 섹션 작성: PR 매트릭스 · SSOT 파일 인덱스 · 새 API surface · 컴포넌트 트리 · 환경 변수 · 다음 우선순위 · 알려진 한계+guard · memory 인덱스 · 한 줄 시작 가이드.
- **memory 미러** — `memory/project_<slug>_handoff.md` 자동 미러. plan 에 `handoff` verb 있으면 end-user dossier bundle 도 생성(클릭 한 번에 받아 나갈 산출물).
- command + marketplace + README 0.6.0→0.7.0 lockstep 보정 (g22).
