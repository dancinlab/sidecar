...
|---|---|---|---|
| [`commons`](hooks/commons/) | hook | 0.9.2 | SessionStart + PreCompact + PostCompact hook — injects a cross-project `do` / `dont` layer (from `commons.tape`, granular `@D g1..g23`) above the per-project context. PostCompact re-injects after the `※ recap` so the layer survives auto-compaction. |
...
| [`cloud`](skills/cloud/) | skill + command | 0.1.0 | `/cloud <args>` — wraps `hexa-cloud` (runpod dispatch · **separate binary** at `hexa-lang/bin/hexa-cloud`, NOT a `hexa cloud` subcommand). Subverbs: `run` · `nohup` · `poll` · `copy-to` · `copy-from`. Fails fast with install hint if not on PATH. NL trigger (*"GPU pod 에 돌려"*, *"runpod dispatch"*). Pairs with `commons g8` + `g12`. |
