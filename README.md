...
| [`plan-guard`](hooks/plan-guard/) | plugin (linter + hooks) | `core` | 0.1.0 | step-by-step 계획의 **locked decision 준수** 강제 — marker(기계가독 contract) + hook(verbatim 주입) + linter(ship 검증) 3종. CORE: `plan-lint` 엔진이 plan.md `## locked decisions` 의 `@L<n> … assert:<grep\|file\|verdict> <arg>` (step-by-step 0.8.0 marker) 를 파싱 → repo 또는 `--diff` 파일셋에 대해 검증 → `@L` 별 ✓/✗ + 미충족 시 nonzero. `@L` 없는 plan 은 skip(후방호환). advisory only(번들 hook 은 additionalContext 경고만). memory-lint hexa-lint 패턴 복제 |
