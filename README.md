...
|---|---|---|---|
| [`commons`](hooks/commons/) | hook | 0.9.12 | SessionStart + PreCompact + PostCompact hook — injects a cross-project `do` / `dont` layer (from `commons.tape`, granular `@D g1..g27`) above the per-project context. PostCompact re-injects after the `※ recap` so the layer survives auto-compaction. Carries [`LATTICE_POLICY.md`](hooks/commons/LATTICE_POLICY.md) (real-limits-first SSOT). |
...
| [`secret`](skills/secret/) | skill + command | 0.1.0 | `/secret <args>` — wraps the [`secret`](https://github.com/dancinlab/secret) CLI (macOS Keychain-backed credentials). Verbs: `get` · `set` · `delete` · `list` · `service`. ⚠ `/secret get` exposes the value in conversation context — prefer inline bash `$(secret get <k>)` for tool-invocation patterns. NL trigger (*"키체인"*, *"토큰 저장"*, *"credential 가져와"*). |
