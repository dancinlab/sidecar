...
| `wilson-resume` | `Stop`·`SessionEnd`·`SessionStart` + `/wilson-resume` | Resume continuity across an abrupt session end — `wilson-checkpoint` restores the *files*, `wilson-resume` restores the *work narrative*: how far the task got and what came next. Every `Stop` snapshots the model's own TodoWrite checklist (read from CC's on-disk `~/.claude/todos/`, so progress is deterministic — no slash command to remember), the last request and the git working tree to a per-project file; `SessionEnd` flips a clean-exit flag (clean stop vs abrupt kill); `SessionStart` (startup/resume/clear) re-injects a short `## Resume` briefing. Progress-only — the goal line stays `wilson-goal`'s, so the two blocks sit side by side — **working**, default ON (opt out: `SIDECAR_NO_RESUME=1`) |
...
/plugin install wilson-resume@sidecar           # resume the work thread after an abrupt session end
...
│   │   ├── hooks/hooks.json          # Stop·SessionEnd·SessionStart
│   │   ├── bin/r.sh                  # hook + command entrypoint
│   │   └── bin/_r.py                 # progress snapshot + resume briefing (working)
│   ├── wilson-inbox/
│   │   ├── .claude-plugin/plugin.json
