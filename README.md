...
| [`step-by-step`](commands/step-by-step/) | command + alias `/sbs` | `core` | 0.3.0 | plan-first sequential runbook · **THREE modes** (question-density `auto──manual──full`): **AUTO** (show plan, run straight through) · **MANUAL** (default·pause+consult per step) · **FULL** (AskUserQuestion rounds until ambiguity=0, then run·re-ask on new mid-run ambiguity). First arg `auto`\|`manual`\|`full`. (`go` is a separate command, not a mode) |
...
| [`mining`](skills/mining/) | command + skill | `core` | 0.2.0 | /mining — lens-driven DIVERGENCE (add leaves) + CONVERGENCE (add edges) workflow, 3rd pillar of /domain. `<NAME>.mining.md` (## cycles · ## leaves · ## edges, append-only) + `<NAME>.mining.tape` (idea cart). TWO cycle kinds — **lens** (divergence; deplete at 0 new leaves) and **connect** (convergence; deplete at 0 new edges in a full pass). Mining = (leaves, edges) graph — divergence builds nodes, convergence compresses to underlying truth. Bundled lenses: **same-formula** · **ouroboros** (goal closure) · **dimensional** · **tension** · **combinatorial** · **custom**. Verbs (10): bare (status) · `<lens>` (round) · `append` · `cycle new` · `depletion` · `tree` · `connect`/`edges` (convergence round) · `connect <a> <b>` (pair justify or (no-edge)) · `graph` (ASCII + stats) · `saturate` (auto-loop connect until depletion). Active-domain only (commons @D g58) |
