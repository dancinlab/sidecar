...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.8.4 | PreToolUse(Bash) pool auto-router + sign-local single-gate. **0.8.4 terminology**: 'WHITELIST' = **LOCAL-EXECUTION** allowlist (things that STAY local: $CLAUDE_PLUGIN_ROOT · hexa cloud · hexa atlas register · /Users//home abs-path · ~/. dotstate · git/gh · npm/pnpm/yarn · `! sidecar sign local` 5min token) — registration is **sign-gated** by design (project.tape `sidecar sign` for hardcoded edits · TUI `!` bang for runtime force-local · no agent self-mint). The old 'HEXA WHITELIST' phrase from 0.8.0 was misleading and is now called **HEXA POOL-DISPATCH** (the all-hexa-routes-to-pool rule). **0.8.3** — **POOL-CANONICAL `~/.hx/canon`**: pool dispatch pins hexa to a pool-owned `origin/main` checkout via `HEXA_REAL_BIN` + `HEXA_LANG=$HOME/.hx/canon` + `HEXA_DAEMON=0`, solving stale-compiler / qwen_bpe miscompile (bootstrap via `~/.hx/canon-update.sh`). Auto-self-heal deferred |
