...
| [`draft`](skills/draft/) | command + skill | `core` | 0.1.0 | Ephemeral scratchpad — `/draft <slug>` scaffolds `drafts/<slug>.md` for temporary working notes designed to be discarded; `drafts/` is auto-gitignored so files never commit by accident; distinct from `/inbox` (typed taxonomy for eventual upgrade) |
...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.11 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.11 — useless env-prefix advisory (agent education): when SIDECAR_NO_POOL/POOL_DISABLE/etc env-prefix is detected, emits inline `additionalContext` explaining no-op since 0.7.3 + points to `! sidecar sign local` as the only legitimate local-force; routing behavior unchanged |
