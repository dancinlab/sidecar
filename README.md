...
| [`monitor-guard`](hooks/monitor-guard/) | hook | `core` | 0.1.4 | PreToolUse(Bash) advisory for bg/long shell launches — detach + log + Monitor-on-log (commons @D g10); 0.1.4 — `has_detach` token-position (nohup/setsid at command position, disown as standalone), same shape as the 0.1.3 cloud-bridge fix — kills `grep ... nohup ...` style [log] false-positives |
