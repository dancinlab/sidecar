...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.5 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.5 — routing-rate baseline counters (POOL-OFFLOAD m1): every meaningful classifier verdict increments one of 4 counters in `~/.pool/route-counters.tally` (routed · local_bound · light_allowed · heavy_failed) so `/check` reports the % of bash invocations actually offloaded vs stayed local |
