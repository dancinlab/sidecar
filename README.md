...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.8.1 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.8.0 — **HEXA WHITELIST** (POOL-OFFLOAD m8): ANY hexa execution routes to pool (replaces the heavy-verb blacklist that leaked verify/parse/atlas → Mac fork-storm). 4 structural local-exemptions ($CLAUDE_PLUGIN_ROOT · hexa cloud · abs-path sign-gate · `hexa atlas register` 0.8.1 local-SSOT write); non-hexa heavy classifier (make/cargo/gcc/…) unchanged |
