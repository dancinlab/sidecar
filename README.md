...
| [`step-by-step`](commands/step-by-step/) | command + alias `/sbs` | `core` | 0.3.0 | plan-first sequential runbook · **FOUR modes** (질문밀도 `go──auto──manual──full`): **GO** (no plan gate·terse 즉시) · **AUTO** (계획 후 무중단) · **MANUAL** (기본·단계별 pause+consult) · **FULL** (모호함 0까지 AskUserQuestion 반복 후 실행·mid-run 새 모호함시 재질문). First arg `go`\|`auto`\|`manual`\|`full` |
