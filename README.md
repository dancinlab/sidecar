...
| `ing {show\|add\|next\|done\|inject\|staleness-check\|stop-check}` | 진행보드(`ING.jsonl` · 전용 `ing` git ref) — `add`/`next`/`done`(완료=scrub→CHANGELOG)·`inject`(매 턴 보드+턴마감 게이트 주입). **GPU 팟 추적은 이관됨** — 옛 `ing pod` 은 공용 `~/.sidecar/pods.jsonl`(`sidecar pod`)로 빠졌고, ING 은 그 파일을 읽어 실행중 팟만 inject/`show` 에 표면 표시(store 소유 없음). **`stop-check`(Stop 훅 · `decision:block`)** — **매 응답에 `🔄 ING` 줄**(`🔄 ING 갱신: …` 또는 `🔄 ING: 변동 없음`)이 없으면 차단해 강제. 코드편집 아닌 **측정·verdict·벤치·에이전트 착륙** 진행도 포착(파일-edit 무관·response-marker 기반 · recommend stop-check 동형) · sidecar-managed repo 한정 · `stop_hook_active` loop-guard 체인당 1회. `staleness-check`(코드 ≥editThreshold 편집인데 보드 무변동 = warn-only)는 보조 신호 | UserPromptSubmit + Stop |
...
| `pod {add\|rm\|list\|watch\|unwatch\|poll}` | GPU pod **로스터 + 자동 폴링** — **단일 공용 SSOT `~/.sidecar/pods.jsonl`**(호스트 전역 append형 jsonl · pool.json 동형 위치 + ING.jsonl 동형 포맷). 로스터와 watch/poll 설정이 **같은 엔트리**에 산다: 옛 `ing pod`(ING `ing` ref 의 `kind:"pod"`)와 옛 `pod-watch.json` 키드 레지스트리를 하나로 통합 — 팟은 더는 ING 에 저장되지 않고, ING inject 는 이 파일을 **읽어** 실행중 팟만 표면에 띄운다. `add <id> <provider> <gpu> <purpose> [cost]` = 로스터 등록(폴링 없음) · `rm <id>` = 로스터에서 제거 · `list` = 로스터+폴링상태. `poll <id>` = one-shot **read-only**(alive→GPU-util/`--ssh-check`); `--teardown-on-done`/`--pull "<remote> <local>"` 있을 때만 회수 후 teardown(**pull-then-destroy**, a_fire_recover_complete · 성공 teardown = 로스터에서 자동 제거). `watch <id> [--interval 600] [--cron]` = cadence 폴링 표시(엔트리 없으면 생성): `--cron`(crontab `*/10`) 또는 agent `ScheduleWakeup` fallback · `unwatch` = 폴링만 중단(**로스터 엔트리는 유지** · 제거는 `rm`). 모든 원격 op 은 `hexa cloud` 경유(`alive`·`exec`·`copy-from`·`rm` · 직접 ssh/curl 금지) | GPU pod 로스터·폴링·회수 |
