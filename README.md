...
|---|---|---|---|
| [`commons`](hooks/commons/) | hook | 0.7.2 | SessionStart + PreCompact + PostCompact hook — injects a cross-project `do` / `dont` layer (from `commons.tape`) above the per-project context. PostCompact re-injects after the `※ recap` so the layer survives auto-compaction. |
...
| [`bypass`](skills/bypass/) | skill | 0.1.1 | **Default-on** — kill the "next user action" punt. Before emitting any user-action block, the agent self-checks; if commands are agent-executable (no human input · no unauthorized destructive ops · no external messages), just run them. User does NOT need to invoke. Cross-project always-on guard in `commons.tape` ≥ 0.7.2. |
