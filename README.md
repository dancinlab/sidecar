...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.6 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.6 — heavy hexa verb sweep (POOL-OFFLOAD m6): heavy_pairs +19 verbs — `hexa test`/`bench`/`absolute`/`meta-closure`/`smash`/`free`/`hyperarithmetic`/`chain` + 11 drill variants (omega·surge·dream·swarm·reign·molt·wake·forge·canon·debate·revive) — all fork-storm class per the hexa 80+ verb catalog |
