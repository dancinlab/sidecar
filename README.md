...
| _(convergence = 재발방지 학습 store)_ | 구 `convergence` 명령 폐기 — 학습은 `ARCHITECTURE.json` 의 `convergence.records[]`(id·state·value·threshold·source) 단일 SSOT 에 산다(인라인 `@convergence` 마커 폐기). 에이전트가 그 `source` 파일을 **Read/Write/Edit 할 때마다** PreToolUse 가 매칭 학습을 주입(같은 결함 재도입 차단) · `sidecar lint` 가 레코드 well-formed(id+state) 강제 | 파일 터치 시 자동 · commit |
