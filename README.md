...
| [`step-by-step`](commands/step-by-step/) | command + alias `/sbs` | `core` | 0.4.0 | plan-first sequential runbook · **THREE modes** (question-density `auto──manual──full`): **AUTO** (show plan, run straight through) · **MANUAL** (default·pause+consult per step) · **FULL (0.4.0)** disambiguate in **CHAT FORM** (1Q/round with easy-mode 7-element scaffold — icon·name·alias·plain-line·analogy·ASCII·compare-table·recommendation; free-form answers OK) until ambiguity=0, then `🎯 합의된 결정셋` ASCII tree pause (user `go` or `Qn=<other>` to flip just that one); re-ask in chat + re-render on new mid-run ambiguity. First arg `auto`\|`manual`\|`full`. (`go` is a separate command, not a mode) |
