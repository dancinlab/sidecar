...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.8.2 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.8.0 — **HEXA WHITELIST**: ANY hexa execution routes to pool (4 structural local-exemptions: $CLAUDE_PLUGIN_ROOT · hexa cloud · abs-path · `hexa atlas register`); 0.8.2 — preflight drops obsolete `build/hexa_interp` probe (hexa is COMPILED, interpreter deprecated → readiness = `command -v hexa` + `hexa --help`, no more wrongly-skipped hosts); non-hexa heavy classifier unchanged |
