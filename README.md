...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.8.3 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.8.0 — **HEXA WHITELIST**: ANY hexa execution routes to pool (4 structural local-exemptions: $CLAUDE_PLUGIN_ROOT · hexa cloud · abs-path · `hexa atlas register`); 0.8.3 — **POOL-CANONICAL `~/.hx/canon`**: pool dispatch pins hexa to a pool-owned `origin/main` checkout (independent of each host's `~/core/hexa-lang` feature branch / WIP), injected via `HEXA_REAL_BIN` + `HEXA_LANG=$HOME/.hx/canon` + `HEXA_DAEMON=0`. Solves stale-compiler / qwen_bpe miscompile class (ubu-2 was 614 commits behind on stale main + DNS-broken). Bootstrap via `~/.hx/canon-update.sh` per host (idempotent: git fetch + reset --hard + bootstrap-C copy + `hexa cc --regen`). Auto-self-heal deferred to 0.8.4 |
