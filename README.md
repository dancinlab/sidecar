...
| `architecture {inject\|show\|search <q>\|lint\|convergence …}` | repo-root `ARCHITECTURE.json`(우선)/`.md` 의 **스켈레톤**(title+summary+2단계 목차 · ~5KB)을 컨텍스트로 주입 — 대용량 정적-doc canonical 패턴: 트리는 commons 처럼 상주하되 **세부(`상세`셀·전체 트리·convergence)는 파일에 두고 `show`/`search`/Read 로 온디맨드 pull**(매-턴 풀-주입 안티패턴 회피 — 10KB additionalContext 한도 초과→하네스 파일-폴백+매턴 토큰 재지불). **정적 doc 이라 per-turn 아님**: SessionStart + PreCompact/PostCompact 만(부재 시 무음 — 부재 repo 를 깨우는 🌱 부트스트랩 씨앗은 `turn-close inject` 담당). 스켈레톤 **뒤**에 턴-마감 게이트.  | SessionStart + Compact + Stop |
| `turn-close {check\|inject}` | **턴 마감 트리오 게이트** — 매 응답이 `🔄 ING` · `🏛️ ARCHITECTURE` · `🧬 CONVERGENCE` **세 줄을 함께** 달도록 강제하는 단일 Stop 훅. 각 줄은 `갱신`/`기록` 주장 아니면 명시적 무변동(`변동 없음`/`해당 없음`). **키워드 스캐너 4종(`ing stop-check` · `architecture stop-check` · `convergence stop-check` · `gate-stop-check`)을 대체** — 신호 스캔은 응답이 쓰인 *뒤* 예측불가하게 난입했고, `stop_hook_active`(anti-wedge) 때문에 **첫 게이트만 차단하고 나머지는 스킵** = 트리오의 2/3 이 구조적으로 강제 불가였다. 이제 게이트 1개 · block 1회 · reason 1개에 누락/위조 leg 를 전부 나열 → 한 번의 re-turn 으로 복구. **주장은 검증된다**(마커만 쓰는 자기보고 위조 차단 · commons verify-done): ING=`ing` ref 전진 · ARCHITECTURE=파일 diff(working·staged·직전 커밋) · CONVERGENCE=`records[]` id + diff. 무변동 확인은 검증 안 함(false-positive 회피). `🔬 GATE` 마커도 함께 폐기 — 게이트 verdict 는 곧 ARCHITECTURE.json `type:"gate"` 노드 편집이라 🏛️ 줄이 이미 커버(single-doc). **활성 leg 없으면(설계트리·보드 둘 다 없음) 완전 무음** → 평범한 repo 는 안 건드림. `inject`=매턴 트리오 지시 + `ing` ref 베이스라인 스냅샷 + **🌱 부트스트랩 씨앗**(저장소 **부재 검사** 결과에 따라 1줄씩 — leg 는 저장소 존재로만 켜지므로 새 repo 에선 규율 자체가 침묵했다. **⓪ `harness.config.json` 없음/깨짐 → `sidecar init --config-only` 요구 + ①~③ 억제**(기본값만 남으면 `verify.checks`=[] 라 `sidecar ci` 가 공허한 green · 나머지 씨앗의 옵트아웃도 이 파일 안에 산다 · 별도 knob 없음 = 파일 생성이 곧 opt-out) ① `ARCHITECTURE.json` 없음 → 트리 작성 요구 ② `ARCHITECTURE.md` 만 있음 → 🧬 store 가 JSON 에만 있으므로 JSON 승격 요구(①② 옵트아웃 `archSeed:false`) ③ `refs/heads/ing` 없음 → **이번 일이 다단계면** `sidecar ing add` 로 보드 개설 요구(옵트아웃 `ingSeed:false` · 무조건 명령 아님 — 줄 끄려고 만든 가짜 보드는 게이트를 허구에 무장시킨다). 저장소 생기면 자동 소멸=0B · 엔진은 절대 스캐폴드 안 함 — 존재만으로 leg 가 무장돼 placeholder 가 게이트를 속이므로, 에이전트가 실제 트리를 쓰고 남은 스텁은 `ARCH-PLACEHOLDER` 가 차단) | 턴 마감 3줄 보고 강제 |
...
| `architecture convergence {list\|for\|add\|rm\|edit}` | 재발방지 학습 store CRUD — 학습은 `ARCHITECTURE.json` 의 `convergence.records[]`(id·state·value·threshold·source) 단일 SSOT 에 산다(구 `convergence` 명령·인라인 `@convergence` 마커 폐기). `add`=**`--source` 필수**(학습은 그 파일에 키잉됨) + **id 자동 부여**(source 파일명 slug `<base>-<n>` · `--id` 로 upsert override) + **검토-우선 게이트**(동일 source 에 기존 레코드가 있으면 BLOCK — 무작정 append 금지 → 먼저 `convergence for <file>` 로 꺼내 보고, 같은 결함이면 `edit <id>` 로 갱신·정말 별개면 `--new` 로 의식적 추가 = 중복·미분류 누적 방지) · `for <file>` 그 파일의 학습 조회(읽기전용 · 터치 시 inject 와 동일 매칭) · `edit <id>` 부분 패치 · `rm <id>` · value/threshold 셸특수문자는 `--value -` stdin. 재발-신호 **키워드 스캐너는 폐기**(`stop-check` 삭제 · 신호가 턴을 가로채고, `stop_hook_active` 때문에 트리오 중 첫 게이트만 강제됐다) → 기록 강제는 이제 `turn-close check` 트리오의 🧬 줄이 담당(매 응답 필수 · `기록: <id>` 주장은 records[] id + diff 로 위조 검증). 파일-터치 시 학습 주입(`convergenceForFile`)은 **활성**(`convergenceOnTouch` 기본 ON) — `preWrite` 가 그 파일에 키잉된 학습을 PreToolUse `additionalContext` 로 inject(stderr 아님 · 비-block 경로만 · 같은 결함 재도입 차단) · `sidecar lint` 가 레코드 well-formed(id+state) 강제 · `architecture inject` 스켈레톤은 records 제외(세부는 `convergence list` 온디맨드) | 재발 학습 기록 + Stop 트리거 |
| `ing {show\|add\|next\|done\|inject\|staleness-check\|stop-check}` | 진행보드(`ING.jsonl` · 전용 `ing` git ref) — `add`/`next`/`done`(완료=scrub→CHANGELOG)·`inject`(매 턴 보드+턴마감 게이트 주입). **`stop-check`(Stop 훅 · `decision:block`)** — **매 응답에 `🔄 ING` 줄**(`🔄 ING 갱신: …` 또는 `🔄 ING: 변동 없음`)이 없으면 차단해 강제. 코드편집 아닌 **측정·verdict·벤치·에이전트 착륙** 진행도 포착(파일-edit 무관·response-marker 기반 · recommend stop-check 동형) · sidecar-managed repo 한정 · `stop_hook_active` loop-guard 체인당 1회. `staleness-check`(코드 ≥editThreshold 편집인데 보드 무변동 = warn-only)는 보조 신호 | UserPromptSubmit + Stop |
...
| `dojo [<slug>]` | dojo 학습잡 스캐폴더. 기본 스택은 `config.dojo`(엔진 무하드코딩)가 운반 — 설정 시 `hexa dojo <delegate>` 위임. 다샤드 배치는 `hexa cloud fire-shards` (손수 launcher.sh 는 `CLOUD-HANDROLLED-FANOUT` warn 으로 리다이렉트) | 학습 스캐폴드 |
...
> 🧪 **프론티어 모델 위임은 `sidecar lab [fable|sol|full]`**: 세션 백엔드를 안 바꾸고 지시 1건을 per-call 위임. 백엔드는 **선택** — 생략하면 **full**(기본값)=둘 병렬(`── fable ──`/`── sol ──` 섹션) · **fable**=Claude Fable 5(`claude -p`·claude-fable-5·opus fallback 금지) · **sol**=OpenAI Codex 5.6(`codex exec`·gpt-5.6-sol). 백엔드 이름 근접오타(`fabel`)는 프롬프트로 오인 않고 거부. 프롬프트=argv/`--file`/stdin→자식 stdin(argv 누출 0). 기본 investigate(읽기·분석만)·`--write`=구현 tier · `--bg`→잡ID(`result`/`tail`/`wait`/`list`). `sidecar lab-mode on [fable|sol|full]`(기본 full) 이 매턴 이 명령으로 설계/분석을 위임한다(구현은 로컬) — **리포별 토글**: 스위치는 그 repo 의 `harness.config.json` `labMode` 문자열 하나뿐(`off`|`fable`|`sol`|`full` — 백엔드 이름이 곧 on 스위치라 모순 상태가 불가)이고 **호스트 전역 스코프는 없다**(미지정=off=무주입 · 원하는 repo 마다 개별 opt-in).
