...
<!-- LATEST-SHIP -->
2026-05-24T12:38Z · chore(docs): 슬래시 명령 네임스페이스 표기 제거 + README step-by-step backfill + quota README 0.7.0 현행화

command(commands/*.md)는 유저 타이핑 시 항상 plain /이름 — 네임스페이스는
모델 Skill-도구 호출 + 자연어 트리거 표면에만 존재. 문서만 /research:arxiv·
/research:yt·/quota:quota 로 적혀 불일치였던 것을 전부 정정 (32개 명령 충돌 0 확인):
/research:arxiv→/arxiv · /research:yt→/yt · /quota:quota→/quota · /quota:<verb>→/quota <verb>.
루트 README 에 직전 step-by-step 0.1.0 누락분 backfill (카탈로그 + 플러그인 표).
quota README 0.4.1 picker 잔재 → 0.7.0 단일 디스패처 현행화.
동작·버전 변동 없음 (marketplace↔plugin.json 정합 유지). 사용자 요청.
...
/cycle-loop           /cycle handed off to /loop — rounds fire automatically
/cycle-full-loop      /cycle-full once, then /loop /cycle for subsequent rounds
/step-by-step <task>  sequential runbook: plan → auto-run steps in order (alias /sbs)
...
| [`research`](skills/research/) | skill + commands |  0.2.2  | Research-fetch tools — `/arxiv <query\|id>` searches the official arXiv API (title · authors · abstract · pdf), `/yt <url-or-id>` extracts YouTube caption transcript via the InnerTube ANDROID client. Implemented in hexa-lang (`hexa run`); HTTP via curl, no API key. |
