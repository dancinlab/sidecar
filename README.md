...
| `convergence {status\|recompute\|by-category\|scan}` | incident 수렴 추적 + **`scan`** = 인라인 `@convergence` 재발방지 마커(c1) 검증(필수키 state·id + state enum) — `harness lint` 가 호출해 불량을 `CONVERGENCE-MALFORMED`(block)로 커밋 차단. 재발 신호(재발·stale·regression·again…)는 keyword 트리거가 마커 작성 환기 | 버그 수정 후 · commit |
