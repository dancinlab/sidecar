...
| [`bypass`](skills/bypass/) | skill | 0.1.0 | Bypass the "next user action" punt — when the agent is about to write a block of bash commands the agent itself can run, just run them. Triggers on natural language ("bypass" · "그냥 해" · "그냥 진행해") or self-recognized punt-pattern. Cross-project always-on guard in `commons.tape` ≥ 0.7.2. |
