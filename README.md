...
| [`mining`](skills/mining/) | command + skill | `core` | 0.1.0 | /mining — lens-driven divergence + pruning workflow, 3rd pillar of /domain (snapshot · log · MINING). `<NAME>.mining.md` (cycle-by-cycle lens tree, append-only) + `<NAME>.mining.tape` (idea cart of @X promotion candidates). Bundled lenses: **same-formula** · **ouroboros** (self-reference fixed-point = goal closure) · **dimensional** (abstraction-rung analogs) · **tension** (contradiction fork) · **combinatorial** (A × B) · **custom** (extensible via `~/.sidecar/lens/<name>.md`). Verbs — bare (status) · `<lens>` (round) · `append <text>` · `cycle new <title>` · `depletion` · `tree`. Active-domain only (commons @D g58) |
