...
| [`git-guard`](hooks/git-guard/) | hook | 0.2.0 | PreToolUse(Bash) deny — blocks force-type git push only: `git push --force` / `-f` · `--force-with-lease` · refspec-level force `+<ref>`. Hook-bypass (`--no-verify`) NOT blocked (left to user discipline). Opt out via `SIDECAR_NO_GIT_GUARD=1`. |
...
| [`tape-lint`](hooks/tape-lint/) | hook | 0.1.0 | PreToolUse(Edit\|Write) deny for `.tape` edits that introduce `@D` blocks with fields other than `do` / `dont`. Diff-aware — pre-existing fields are grandfathered, only newly-introduced fields (`why` · `tool` · `note` · `ref` · ...) block the edit. Per-project enforcement across any repo. Opt out via `SIDECAR_NO_TAPE_LINT=1`. |
| [`hexa-native`](hooks/hexa-native/) | hook | 0.1.0 | **PreToolUse(Write\|Edit\|NotebookEdit) hard block** — `.py` / `.sh` writes are **denied** inside any project rooted at a directory containing a `project.hexa` marker. Reason message redirects the operator to `.hexa` (since `.py` / `.sh` are already supported as ai-native English elsewhere). Targets only `.py` / `.sh`; other extensions pass through. Non-hexa-native projects unaffected. **NO opt-out** by design — no env var, no config file, no exception list; uninstall the plugin if you need a way out. |
