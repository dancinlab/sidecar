...
```
sidecar/
├── bin/sidecar               # CLI — init · sync · mirror · sign · profile · enable · disable · reset · paths · reap · handoff · master
...
/sidecar:sidecar handoff <args>   cross-repo handoff registry (~/.sidecar/handoff/ JSONL) — add · ls · done
...
#   secret-guard   `secret set <K>` / `<tool> login` when the key already exists in `secret list` → resolve via `secret get` + check aliases (g77)
...
79 plugins across `{hook · command · skill}` — one concept each (35 `core` · 18 `hexa` · 24 `personal` · 2 `master`). The **Tier** column is the [enable profile](#profiles) a plugin belongs to.
...
| [`plan-guard`](hooks/plan-guard/) | plugin (linter + hooks) | `core` | 0.3.2 | step-by-step 계획의 **locked decision 준수** 강제 — marker(기계가독 contract) + hook(verbatim 주입) + linter(ship 검증) 3종. CORE: `plan-lint` 엔진이 plan.md `## locked decisions` 의 `@L<n> … assert:<grep\|file\|verdict> <arg>` (step-by-step 0.8.0 marker) 를 파싱 → repo 또는 `--diff` 파일셋에 대해 검증 → `@L` 별 ✓/✗ + 미충족 시 nonzero. `@L` 없는 plan 은 skip(후방호환). **0.2.0** PreToolUse(Task\|Agent) inject hook — sub-agent spawn 시 활성 plan(`status: active`)의 `@L` contract 를 verbatim 주입 + spawn 프롬프트 escape(skip/생략/… near lock term) 경고. **0.3.0** PostToolUse(Bash) ship-lint hook — `git commit`/`gh pr` 직후 활성 plan 의 @L 을 ship diff 에 대해 검증, 미충족 @L 을 advisory 경고(원계획 drift = 근본원인 #2 ship 시점 포착). **0.3.1** 활성추적 강화 — newest-mtime fallback 이 closed(`status: done`/`abandoned`) plan 을 skip (sbs 0.8.1 closure flip 과 짝). **0.3.2** dogfood self-test 문서화(빌드 plan @L1~6 PASS). advisory only(번들 hook 은 additionalContext 경고만). memory-lint hexa-lint 패턴 복제 |
...
| [`secret-guard`](hooks/secret-guard/) | hook | `core` | 0.1.0 | PreToolUse(Bash) ADVISORY hook (commons @D g77) — on `secret set <K>` where `<K>` exists, or a `<tool> login` / `aws configure` where a matching key exists in `secret list`, emits non-blocking additionalContext (resolve via `secret get`, check aliases huggingface.token↔hf.token) so the agent never tells the user to register a credential that already exists; never deny; key NAMES only, values never read |
| [`sidecar`](commands/sidecar/) | command | `core` | 0.8.1 | thin wrapper over the `sidecar` marketplace CLI (host-local, on PATH via `hx install sidecar`). **0.8.1 — `mirror` skips orphaned plugins** (`do_mirror` now selects the highest NON-orphaned cache version; a fully-superseded plugin — every cache version carrying `.orphaned_at` — is never command-mirrored, so a renamed plugin's stale command stops double-surfacing in the TUI; generic, no plugin name hard-coded; fixes the `system`→`lab` rename leaving `~/.claude/commands/system.md` regenerated on every sync). **0.8.0 — `handoff`** (cross-project handoff registry `~/.sidecar/handoff/handoff.jsonl`, the single SSOT for cross-repo gap/patch/bug/RFC handoffs per commons g11·g36·g48·g59·g60 — retires the per-repo INBOX domain; `handoff add <repo> <text...>` · `handoff ls [<repo>]` · `handoff done <id>` · bare = counts summary; JSONL not markdown, repo-external, portable via $HOME). **0.7.0 — `reap`** (reap stale `hexa-lsp` idle > 2h; dry-run by default, `reap --kill` terminates; never touches claude @D s9 or active hexa runs) **+ `paths add/rm` now SIGN-GATED** (key `paths`: `! sidecar sign paths` first, since the whitelist overrides pool-route). **0.6.0 adds `sidecar paths`** — manage the LOCAL-EXECUTION WHITELIST `~/.sidecar/local-paths` (pool-route 0.10.0 reads it). `paths` (list) · `paths add` (register cwd) · `paths add <dir>` (abs-resolved dir, refuses non-existent / already-present) · `paths rm <prefix>`. Seeded on first use with `~/.claude/plugins/cache/sidecar/` + `~/core/sidecar/`. Atomic writes (tmp+mv); whitelist itself IS the opt-in — no env opt-out (@D s11). |
| [`step-by-step`](commands/step-by-step/) | command + alias `/sbs` | `core` | 0.8.1 | plan-first sequential runbook · **TWO modes** + **0.6.0 자동 QA 4축**: **MANUAL (default)** — chat-form disambiguation (1Q/round, easy-mode 7-element scaffold — icon·name·alias·plain-line·analogy·ASCII·compare-table·recommendation; free-form answers OK) until ambiguity=0, then `🎯 합의된 결정셋` ASCII tree pause (user `go` or `Qn=<other>` to flip just that one), then write `drafts/<slug>-plan.md` (gitignored) + **launch background Agent** that runs end-to-end including ship + **auto-QA 4축** — user can leave · **AUTO** same chat-form scaffold rendered but each question AUTO-PICKED by 4-axis weighted average (완성도·단순·안전(blast radius)·표준(sidecar pattern); default 1:1:1:1, inline override `/sbs auto:safety <task>` or `/sbs auto:complete=2,simple=3 <task>`) — still pauses at agreement screen. **0.6.0 자동 QA 4축** (handoff agent ship 직후) — functional (새 endpoint 응답) · visible (URL/path 노출) · conformance (locked decision ↔ 코드 1:1) · regression (기존 surface 미손상). Hybrid fail: regression FAIL → `git revert <SHA>` 자동 + banner; 나머지 FAIL → ship 유지 + plan.md `## qa-deferred` + banner; SKIP=PASS-equivalent. `legacy-manual` = the prior per-step pause behavior (1-version deprecation banner). (`go` is a separate command, not a mode). **0.7.0** — ship+QA 직후 자동 HANDOFF.md 9-section 인계 doc + `memory/project_<slug>_handoff.md` 미러 (plan에 handoff verb 면 end-user dossier bundle 도). **0.8.0** — plan.md frontmatter `status: active` stamp + `## locked decisions` 기계가독 contract `- @L<n> (<axis>): <option> · assert:<grep\|file\|verdict> <arg>` (신규 `plan-guard` 플러그인이 verbatim 주입 + ship 시 `plan-lint` 검증; `@L` 없으면 skip = 후방호환). **0.8.1** plan 라이프사이클 — write 시 `status: active` stamp(1개만), closure 시 `done`/`abandoned` flip |
...
| [`cloud-guard`](hooks/cloud-guard/) | hook | `hexa` | 0.4.0 | PreToolUse(Bash·Write·Edit·MultiEdit) hard block for raw rented-GPU pod dispatch (commons @D g8) + direct writes to the M5 pod registry `~/.hx/cloud/active-pods.json` (route via `hexa cloud`) |
...
| [`micro-exp`](skills/micro-exp/) | command + skill | `hexa` | 0.4.0 | context-driven micro-experiment sweep orchestrator (**domain-agnostic** — kind is abstract; materials wall · LLM bench · web smoke · build bench are examples among many). self-enumerate → **Stage 1.5 per-candidate infra existence check (HALT to `/cycle-bg <domain>` when any of `<runnable>`/`<inputs>`/`<parser>`/`<workdir>` missing)** → pod budget → monitor → parse → atlas auto-fold on closed-form-atom-shaped kinds (skip for observation-only). Cross-links the `/cycle-bg` ↔ `/micro-exp` build-vs-fire decision tree in both SKILL.md (BUILD lane vs FIRE lane). Local-pool adapter works on ANY kind with a runnable + parser |
| [`hexa-loop`](skills/hexa-loop/) | command + skill | `hexa` | 0.1.0 | /hexa-loop — canonical HEXA-LOOP, the universal self-evolving loop. ONE loop drains ANY active domain (math · perf · codegen · web) to depletion via a PLUGGABLE verify-adapter. Each round = DISCOVER (mining-style lens divergence + connect convergence saturate + `hexa kick`/drill breakthrough probe) → 🔌 VERIFY-ADAPTER auto-selected by domain kind {**atlas** (math · `hexa atlas register --from-drill`/`hexa verify`) · **bench** (perf · byte-diff IDENTICAL + Δ) · **byte-diff** (codegen regen) · **smoke** (web liveness)} → ABSORB into the domain SSOT (atlas embed · *.bench.md · .verdicts/ · ledger) → feed verified result as next seed → disk checkpoint → depletion test → ScheduleWakeup 1200s. Preset shorthands `--adapter=atlas` ≈ old `/mirror-loop` · `--diverge-only` ≈ old `/mining`. Adapter extension hook `~/.sidecar/loop-adapter/<name>.md` (mirrors mining's `~/.sidecar/lens/`). ABSORBS + RETIRES `/mining` (discover sub-stage) + `/mirror-loop` (atlas preset); `/kick` KEPT (discover CALLS it). HONEST g63 · IDEMPOTENT dedup · ACTIVE-DOMAIN ONLY g58 (commons) |
