...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.7 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.7 — Node ecosystem (POOL-OFFLOAD m2): heavy_words +4 (`node`·`deno`·`bun`·`ts-node`) + abs-path first-token sign-gate adds same +`tsx` (excluded from substring layer due to `.tsx` file-ext collision); mirrors python/hexa fork-storm protection for JS/TS runtimes |
