...
| 명령 | 역할 | hook 단계 |
|------|------|-----------|
| `pre bash` / `pre write` | **코드레벨 가드**(force-push{blind `--force`/`-f`/`+refspec` 차단 · `--force-with-lease`는 허용 · `# force-ok` 예외} · cloud-raw c11 · poll c19 · danger{`--no-verify`·`reset --hard`·`curl\|sh` 는 상시 always-on · `rm -rf` 루트(`/`·`/*`·`~`·`$HOME`·`*`)는 config `dangerGuard.rmRfRoot` 토글 · **기본 OFF**(opt-out)} · secret-literal c1 · handoff-scatter · naming{버전/복사 접미사 파일·폴더명 `_v2`·`_final`·`_copy`·`foo 2` **BLOCK**(Write/Edit + Bash mv/cp/touch/mkdir) → canonical 단일파일 update-in-place 강제 · 이력은 git history · c25 · `@canonical-ok`(write)/`# canonical-ok`(bash) 면제}) → 그다음 config enforcement 정규식. 코드 가드는 config보다 먼저 실행(profile 편집 무력화 방지) · 인라인 `# …-ok`/`// @secret-ok` 마커 + `dangerGuard.rmRfRoot` 토글만 예외 | PreToolUse |
