...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.9 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.9 — media tools (POOL-OFFLOAD m4): heavy_words +2 (`ffmpeg`·`magick`) + abs-path first-token sign-gate +4 (ffmpeg/magick/convert/sox · convert/sox English-word substring-collide → first-token-only) |
