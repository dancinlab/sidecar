...
| [`git-guard`](hooks/git-guard/) | hook | 0.3.0 | PreToolUse(Bash) deny — blocks force-type git push only: `git push --force` / `-f` · `--force-with-lease` · refspec-level force `+<ref>`. Hook-bypass (`--no-verify`) NOT blocked (left to user discipline). hexa-lang (`_git_guard.hexa`). Opt out via `SIDECAR_NO_GIT_GUARD=1`. |
