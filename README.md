...
|---|---|---|---|
| [`commons`](hooks/commons/) | hook | 0.9.0 |
| [`prefs`](hooks/prefs/) | hook + command | 0.1.0 | User language preferences — 3 axes (`code` authoring · `docs` authoring · `response` to user). SessionStart + PreCompact + PostCompact hook auto-injects. `/prefs show` · `/prefs code <lang>` · `/prefs docs <lang>` · `/prefs response <lang>`. Defaults: code=english, docs=english, response=korean. SSOT for language prefs (commons g2 removed in 0.9.0). | SessionStart + PreCompact + PostCompact hook — injects a cross-project `do` / `dont` layer (from `commons.tape`) above the per-project context. PostCompact re-injects after the `※ recap` so the layer survives auto-compaction. |
...
| [`gap`](commands/gap/) | command | 0.1.0 | `/gap` — multi-axis gap exploration. 40 breakthrough-strategy lenses · 8 families. Bare `/gap` = inline-triage all 40 + deep-dive only hot families (subagents). `/gap full` = exhaustive 8-subagent fan-out. `/gap <scope>` targets the sweep. `/gap list` prints the catalogue. Surfaces + prioritises gaps; never fixes. Restored from pre-v2 `wilson-gap` (`4566a2a`). |
...
| [`domain`](skills/domain/) | skill + command | 0.2.0 | Maintain UPPERCASE `<DOMAIN>.md` (living current-state snapshot, overwrite) + sister `<DOMAIN>.log.md` (append-only history, newest on top) at project root — single invocation writes BOTH. `/domain <NAME> [update]` or NL trigger. Markdown analog of tape v1.2's `<DOMAIN>.tape` + `<DOMAIN>.log.tape` sister pattern. Examples: `ROADMAP.md`+`.log.md` · `STATUS.md`+`.log.md` · `PLAN.md`+`.log.md` · `GOAL.md`+`.log.md` · `DECISIONS.md`+`.log.md`. |
