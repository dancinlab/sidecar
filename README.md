...
| [`cycle`](skills/cycle/) | command + skill | `core` | 0.9.0 | Autonomous work-loop driver (whole family auto-drains to depletion) · 6 commands: /cycle · /cycle-full · /cycle-loop · /cycle-full-loop · /cycle-all (no cap + no recommend gate, fan out EVERY item) · /cycle-fg (0.9.0 — **FOREGROUND SEQUENTIAL**: 같은 enumeration + plan 단계, Stage 4만 인-세션 직접 실행 1개씩 순차 · NO Agent fan-out · halt on failure · 디버깅/조심스러운 리뷰용) |
