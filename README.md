...
| [`all-bg-go`](skills/all-bg-go/) | skill + command | 0.3.0 | Parallel fan-out trigger — "all bg go" → plan table + one background Agent per branch in the same message. **Two modes**: prior-turn (fan out branches the previous turn offered) + **self-enumerate** (no prior branches → derive next viable work items itself, then fan out). Repeatable next-list → fan-out → loop driver. Also `/all-bg-go`. |
