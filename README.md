...
| [`pool-route`](hooks/pool-route/) | hook | `personal` | 0.7.8 | PreToolUse(Bash) pool auto-router + sign-local single-gate; 0.7.8 — JVM compilers + sbt (POOL-OFFLOAD m3): heavy_words +3 (`javac`·`kotlinc`·`scalac`) + heavy_pairs +4 (`java -jar`·`sbt compile`/`test`/`run` — pair-only for short ambiguous names) + abs-path first-token sign-gate +5 (java/javac/kotlinc/scalac/sbt) |
