...
| `wilson-gpu` | `/wilson-gpu` 命令 + `SessionStart` | RunPod / Vast.ai 租用 GPU 成本护栏 —— `SessionStart` 显示计费中实例（运行时长 + 累计估算成本）→ 防止遗忘的 pod 漏钱;`down` 是急停开关，`attach` 把实例接入 `wilson-pool` roster。provisioning（`up`）是**独立开关 · 默认 OFF** —— `up` 需 provisioning ON **+** `--yes` 双重门控。**可用** · 无 `runpodctl`/`vastai` 时 inert（opt out: `SIDECAR_NO_GPU=1`） |
...
| `sidecar` | `/sidecar` 命令（控制） | 其余插件的运行时 on/off —— `/sidecar status\|on\|off <name>`（名称: ssot readme-format hexa-verify dangerous-path git-guard secret-guard bash-guard prefs output-trim pool checkpoint gpu guards，或 `all`）。共享 `~/.claude/sidecar/disabled.json` 由各 hook 检查 · 跨会话持久 · 补充原生 `/plugin` |
