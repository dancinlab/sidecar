...
## `wilson-prefs` 응답 스타일과의 관계

이 원칙이 일반화하는 프로토콜은 자연 축으로 두 가지를 묶는다:

1. **응답 스타일** (예: friendly 7-element 패턴, 추천 포맷, 이모지 enum) — *어떻게 쓰는가*. sidecar `wilson-prefs` 플러그인에 응답 스타일로 별도 존재.
2. **Step-by-step 사용자-확인 게이트** — *이* 원칙, *어떻게 결정하는가*.

**스코프는 독립적**(한쪽이 켜진 채 다른 쪽이 꺼질 수 있다)**이지만, 프레젠테이션은 자동으로 상속된다**:

- 게이트의 옵션 + 추천 + 근거는 **`wilson-prefs`가 현재 선언한 응답 스타일로 렌더링**된다 — 에이전트가 주입된 `## Prefs` 블록(`Active response style: **<name>**`)에서 활성 스타일을 읽어 **사용자가 다시 요청하지 않아도** 적용한다. `wilson-prefs`가 이미 `style=friendly`를 선언했는데 "게이트를 friendly 버전으로 보여줘"라고 수동으로 다시 말해야 했다면 그건 **버그**이지 의도된 워크플로가 아니다.
- 활성 스타일이 **friendly**일 때, 각 옵션과 추천은 **full friendly 7-element 패턴**(이모지 아이콘 · 별칭 · 평문 한 줄 · 일상 비유 · fenced ASCII 다이어그램 · 가장 가까운 도구와 비교)을 쓴다 — bare terse table이 아니다.
- `wilson-prefs`가 **없음 / 미설정 / 비활성**일 때 게이트는 호스트 기본 프레젠테이션으로 폴백한다. 하드 런타임 커플링은 없다 — 게이트는 독립적으로 동작하며, 설정된 적 없는 스타일을 상속할 수 없을 뿐이다.

요약: *게이트를 거치는가*는 이 원칙; *게이트를 어떻게 읽는가*는 활성 prefs 스타일에서 상속되며, 수동 리마인더가 필요 없다.
