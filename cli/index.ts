...
setup:
  ...
  init [--force] [--dry-run] [--config-only]   scaffold THIS repo: config + .harness rules + gitignore + wrapper (hooks are GLOBAL-ONLY → sidecar install)
                                         --config-only = config + wiring 만, 문서 스캐폴드(ARCHITECTURE.json·CLAUDE.md·CHANGELOG.jsonl·state/·ci.yml) 생략 — placeholder 트리는 🏛️/🧬 게이트를 허구에 무장시키므로 설계트리는 에이전트가 직접 작성 (🌱 config 씨앗이 지목하는 모드)
...
hook delegates (wire these into your agent's settings.json):
  ...
  turn-close {check|inject}   턴 마감 트리오 게이트 — 매 응답 끝에 🔄 ING · 🏛️ ARCHITECTURE · 🧬 CONVERGENCE 세 줄 강제 (Stop 훅 · 누락/위조 = decision:block 1회)
                                         ...
                                         inject 는 저장소가 없는 repo 에 🌱 부트스트랩 씨앗을 매턴 발화 — 파일검사로 harness.config.json 부재/깨짐(sidecar init --config-only · 나머지 씨앗 억제 · knob 없음: 파일 생성이 곧 opt-out) · ARCHITECTURE.json 부재(md 만 있으면 JSON 승격 요구 · 옵트아웃 archSeed:false) · ing ref 부재(다단계 작업이면 sidecar ing add 로 개설 · 옵트아웃 ingSeed:false). 저장소 생기면 자동 소멸(0B)
