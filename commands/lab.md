---
description: /lab [fable|sol|full] <지시> — delegate ONE instruction to a frontier model (sidecar lab · per-call). Backend OPTIONAL — omit = full (BOTH parallel · DEFAULT) · fable = Claude Fable 5 · sol = Codex 5.6. Free-text safe — the prompt goes through a FILE. Triggers — "lab", "lab fable", "sol 한테", "파블 시켜", "/lab".
argument-hint: "[fable|sol|full] <prompt — free text, any shell-special chars OK>"
allowed-tools: Bash, Write
---

Delegate the instruction in `$ARGUMENTS` to a frontier model via `sidecar lab`. The FIRST word of `$ARGUMENTS` MAY select the backend (`fable`, `sol`, or `full`); the REST is the prompt — FREE TEXT (quotes, parens, globs, backticks all legal), so it must NEVER be inlined into a shell command line. Instead:

1. **Split** `$ARGUMENTS` — take the first token as `<sub>` ONLY if it is exactly one of `fable`/`sol`/`full`; otherwise `<sub>` = `full` (the default — both models) and the WHOLE of `$ARGUMENTS` is the prompt.
