...
## hexa-lang-model
- do: hexa consumers (`.hexa`) follow SSOT `hexa-lang/docs/lang-model.md`: default = fn-arena auto-manage (no manual free) · `@own` opt-in · `farr_free` handles · determinable types · fix violations
- dont: Manual free/GC on arena values · fn-arena composite into a module-global (dangling — return / build at module scope) · leaking `farr_*` · unboxing default-ON without census · SSOT drift
