...
> 말고 그 트리를 단일 출처로 읽고, 코드/설계 변경 시 lockstep 으로 갱신한다(c13). 사람이 볼 땐
...
바뀌면 보드를 현행화(add/next/done). ING 는 커밋 불필요(빠른 갱신), 의미있는 코드 변동은 c13 로 닫는다.
...
## c13 — every cycle = docs + pr-cycle (필수)
...
## c15 — 벽을 만나면 돌파하라 (closed-negative ≠ 종착) · 벽 분류 먼저
...
- **(e) 투자 부족** — 스케일업(compute/data). 무거운 작업은 c16 대로 pool/`hexa cloud` 로 분산.
...
## c17 — 우회경로는 지시 전 작성 금지 (escape-hatch only on request)
