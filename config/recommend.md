...
- An optional default MODE MAY be set in `.harness/recommend-default` (per-repo, committed = team-shared, wins) or `~/.sidecar/recommend-default` (host-wide, via `set-default --global`) — one token: present · auto · complete · simple · safe · std, OR a `+`/`,`-joined MULTI-axis combo (e.g. `complete+std`); absent = present. The inject hook surfaces the active mode as a `# default mode:` directive.
...
- **FIXED-AXIS** (complete · simple · safe · std) = a STANDING SELECTION is already made, so the box is informational ONLY and is NEVER a stop point. ⚡ BEHAVIORAL MANDATE FIRST: in the SAME turn, AUTO-PROCEED with that axis's champion as the decision and CONTINUE straight into executing it — do NOT end the turn on the box, do NOT ask "진행할까요?", do NOT wait for the user to re-pick. Render the box only for trade-off visibility: ★-mark that axis line IN PLACE (numbering ①②③④ unchanged) + append `  ← 기본값`, STILL render all four lines, then one conclusion line `🤖 고정축 auto-pick: <안> (<axis> 기준)` immediately followed by the actual work.
- **FIXED-MULTI** (`+`/`,`-joined combo, e.g. `complete+std`) = a STANDING MULTI-axis selection — same auto-proceed mandate as FIXED-AXIS, but the pick is SCORED across ONLY the selected axes (each selected axis equal-weight, unselected axes weight 0, tie→안전 if 안전 is selected else the first listed). ★-mark EACH selected axis line IN PLACE + append `  ← 기본값` to every selected line, STILL render all four lines, then one conclusion line `🤖 복수고정축 auto-pick: <안> (<axes> 기준 · 선택축만 채점)` immediately followed by the actual work.
