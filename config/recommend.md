...
spec v2.0 · `harness recommend inject` emits this verbatim each turn as additionalContext.
...
- set via `harness recommend set-default <present|auto|complete|simple|safe|std>` · clear via `clear-default` (→ present) · read via `get-default`.
