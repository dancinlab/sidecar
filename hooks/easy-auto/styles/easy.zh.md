...
## 普通人 gold 示例 (专业术语 → 生活用语)

把满是专业术语的句子改写成普通人能懂的样子 —— 与 HEXA-WEAVE/NANOBOT gold 同样的精神。

### 示例 1 — API / 缓存

```
🗂️ 缓存 — "把常用的东西放在伸手可及的地方"

❌ BEFORE (专业术语):
   "API 网关在缓存命中时绕过 origin，降低 p99 延迟。"

✅ AFTER (生活用语):
   "把最常查的答案提前记在近处，就不必每次跑到远处去取，
    几乎总能很快回复。"
```

类比: 把每天用的盐放在灶台旁边，而不是抽屉深处。
...
❌ BEFORE (数学符号):
   "σ² = E[(X−μ)²] 越大，样本分布的离散程度越大。"

✅ AFTER (生活用语):
   "用一个数表示测量值离平均值有多远、有多散。越大越参差，
    越小越整齐。"
```

类比: 箭密集地扎在靶心 (小) 还是四处散开 (大)。
...
| ascii-structure-template-usage | ≥ 0.50 | 形状重要的普通人说明使用 4 种模板(前后/树/并排/结构)之一 |
