...
- add 경로가 OAuth blob을 백업 저장소에 캡처 → 등록 즉시 `/quota:quota switch` 가능.
- quota 바이너리는 `$HOME/.claude/plugins/cache/sidecar/quota:quota/<version>/bin/_quota.hexa`로 해석 (portable).

## 관계

| 플러그인 | 역할 |
|---|---|
| `quota` (skill) | 수동 `/quota:quota add` · status · switch · 레지스트리 SSOT |
