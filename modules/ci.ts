...
// ── CI scaffold — emit a Blacksmith GitHub Actions workflow that runs the repo's
...
export function ciWorkflowYaml(project: string, runner: string, setup: CiStep[]): string {
  ...
  return `# ${project} CI — runs \`sidecar ci\` (verify.checks) on a Blacksmith cloud runner.
#
# Generated by \`sidecar ci scaffold\` (also written by \`sidecar init\`). Blacksmith
# is a drop-in fast runner (\`runs-on:\` label only). The checks are the repo's own
# verify.checks (config-driven) — edit them in harness.config.json, not here. Change
# the runner / stack setup via config \`ci.runner\` / \`ci.setup\`, then re-scaffold.
...
export function ciScaffold(args: string[]): number {
  ...
  const cfg = config();
  // CI must always run on Blacksmith — the scaffold never emits a github-hosted
  // runner. If config overrode `ci.runner` to a non-blacksmith label, warn loudly
  // (kept advisory, not a hard block, so a deliberate override is still possible).
  if (!cfg.ci.runner.startsWith("blacksmith-")) {
    warn(`ci scaffold: ci.runner='${cfg.ci.runner}' is NOT a Blacksmith runner — CI should always use blacksmith-* (e.g. blacksmith-4vcpu-ubuntu-2204).`);
  }
