...
export async function preBash(_args: string[]): Promise<number> {
  ...
  if (cloudLabel) {
    return emitBlock(
      "CLOUD-RAW-CLI",
      `${cloudLabel} — raw GPU-provider CLI/API direct use is blocked (commons c11). Use the hexa builtin: GPU/cloud → \`hexa cloud run <host> -- <argv...>\`. No override — provider CLIs/APIs are not called directly from the agent.`
    );
  }
