#!/usr/bin/env python3
# wilson-research :: arXiv search / fetch.
#
...
def _get(url):
    req = urllib.request.Request(url, headers={
        "User-Agent": "wilson-research (sidecar plugin)"})
...
def main():
    ...
    if not args:
        print("wilson-research/arxiv: usage: /wilson-research:arxiv "
              "<query | arxiv-id> [--n N]")
        return
    url = build_url(args)
    if not url:
        print("wilson-research/arxiv: empty query")
        return
    try:
        root = ET.fromstring(_get(url))
    except urllib.error.URLError as e:
        print("wilson-research/arxiv: network error — %s" % e)
        return
    except ET.ParseError as e:
        print("wilson-research/arxiv: unexpected API response — %s" % e)
        return
    entries = root.findall(ATOM + "entry")
    if not entries:
        print("wilson-research/arxiv: no results")
        return
