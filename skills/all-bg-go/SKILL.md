...
description: |
  ...
  proceeds concurrently. REACTIVE single fan-out of what the prior turn
  offered. For a self-generating REPEATABLE loop (enumerate next work →
  plan → fan-out → loop), use `/cycle` instead.
allowed-tools: Agent, Bash, Read
---

# all-bg-go — plan-then-fire the prior turn's branches in parallel
...
**Reactive only**: all-bg-go fans out exactly what the PRIOR turn offered. If the user wants the agent to self-generate the next batch of work and loop, that's `/cycle` (the proactive loop driver) — not this skill.
...
- **Do not invent branches.** Only fan out directions the prior turn actually offered. If the prior turn offered one direction (or zero), there is nothing to fan out — point the user at `/cycle` (which self-enumerates the next batch) or ask what they want fanned.
...
- **Do not nest fan-out.** Sub-agents should not themselves invoke `all-bg-go` or `/cycle`.

## Related

- `/cycle` — the proactive loop sibling: self-enumerates the next work batch each round (next-list → plan → fan-out → loop). Use when there's no prior-turn list and you want to keep marching.
