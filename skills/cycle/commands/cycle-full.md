...
3. **Dup-race precheck** — same as `/cycle` step 2: for each item that names a cross-repo handoff entry (in the `sidecar handoff` registry, `~/.sidecar/handoff/handoff.jsonl`), run the 3-signal grep (A: locate the entry via `sidecar handoff ls | grep -iE '<slug>'`, then read its `STATUS` column — a `done` status counts as resolved · B: `gh pr list --state merged --search "<slug>"` · C: `git log --all --grep="<slug>"`). Mark SKIP / PROCEED with reason. Print one judgement line per item.
