...
3. **Dup-race precheck** — same as `/cycle` step 2: for each item that names an inbox patch slug (matches `inbox/**/<slug>.md` in the current repo), run the 3-signal grep (A: patch file `Status:` line against resolved-class regex `(fixed|resolved|closed|landed|shipped|absorbed|superseded|merged|done|✅|🟢)` · B: `gh pr list --state merged --search "<slug>"` · C: `git log --all --grep="<slug>"`). Mark SKIP / PROCEED with reason. Print one judgement line per item.
