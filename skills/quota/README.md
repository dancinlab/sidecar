...
| Access token | `~/.claude/.credentials.json` (Linux) · macOS Keychain svc `"Claude Code-credentials"` (macOS) |
| 계정 ID | `~/.claude.json` → `oauthAccount.{emailAddress,accountUuid,...}` |
| 캐시 | `$HOME/.sidecar/quota/usage_cache_<uuid>.json` (45초 TTL) |
| 레지스트리 | `$HOME/.sidecar/quota/accounts.json` (0.2.0에서 채워짐) |
...
| 현재 (0.1.0) | sidecar `skills/quota/bin/_quota.hexa` (~430줄) | view-only port; wilson harness 결합 제거 |
| 0.2.0 | (예정) `/quota add` | 레지스트리에 현재 계정 메타데이터 스냅샷 |
| 0.3.0 | (예정) `/quota switch`/`remove`/`refresh` | OAuth blob keychain 백업 + live cred 스왑 + rollback |

## 거버넌스

- @D s1 (concept separation): 1 plugin = skill + 그 mechanism인 command
- @D s3 (portable paths): `$HOME`, `$CLAUDE_PLUGIN_ROOT`만 사용
- @D g5 (verify via hexa CLI only): 본 플러그인은 view-only이므로 g5 verdict 대상 아님 (관측 도구)
