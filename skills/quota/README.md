...
`/quota:quota` 슬래시 명령. **등록된 모든 계정**을 6컬럼 표 하나로 보여준다 — 현재 활성 계정(`~/.claude.json`)은 `★` + 비공식 OAuth usage endpoint 라이브 5h/7d(45초 캐시), 나머지 계정은 각자 마지막 캐시값(`—` = 미조회, 절대 가짜값 안 씀).

## 명령 — `/quota:quota <verb>` 단일 디스패처

단일 디스패처 `/quota:quota <verb>` — 동사는 인자로 전달 (`/quota:quota list` · `/quota:quota add` …). bare `/quota:quota` 는 통합 표.

| 동사 | 호출 | 결과 |
|---|---|---|
| (bare) | `/quota:quota` | 통합 표 (= list) |
| list | `/quota:quota list` | **전체 계정 통합 표** — 활성 `★` + 라이브, 나머지 캐시. `status` 는 이 명령의 별칭 |
| status | `/quota:quota status` | `list` 의 별칭 — 동일한 통합 표 |
| all | `/quota:quota all` | 같은 표지만 **모든 계정을 라이브 fetch** (느림 · rate 제한 · "전부 갱신" 1-shot 토글) |
| add | `/quota:quota add [<nick>]` | 현재 계정 등록 + 자격증명 백업 캡처. 닉네임은 **옵션** |
| nick | `/quota:quota nick <ref> [<nick>]` | 기존 계정의 닉네임 설정/해제. 빈 인자 = 해제 |
| switch | `/quota:quota switch <ref>` | 라이브 계정 전환 (검증 + 실패시 rollback) |
| remove | `/quota:quota remove <ref>` | 레지스트리에서 제거 + 백업 cred 삭제 |
| refresh | `/quota:quota refresh [<ref>]` | 사용량 재조회 → 계정별 캐시 갱신 (만료 토큰 자동 갱신) |
| help | `/quota:quota help` | 도움말 |

`<ref>` = **닉네임** · 이메일 · `/quota:quota list`의 인덱스 — 셋 다 통과. lookup 순서: index → nick → email.

### 닉네임 동작

| 입력 | 결과 |
|---|---|
| `/quota:quota add` | 닉네임 없음 — 이메일만으로 식별 (기본값, 이전 0.3.0과 동일) |
| `/quota:quota add claude1` | 등록되면서 `claude1`로 태그 |
| `/quota:quota add claude2` (이미 등록된 계정) | 닉네임만 업데이트 (중복 행 X, cred 재캡처 X) |
| `/quota:quota nick claude1 claude-main` | 이름 변경 |
| `/quota:quota nick claude1` (빈 인자) | 닉네임 해제 → 다시 이메일만 |
...
| 캐시 | `$HOME/.sidecar/quota:quota/usage_cache_<uuid>.json` (45초 TTL) |
| 레지스트리 | `$HOME/.sidecar/quota:quota/accounts.json` |
| **백업 cred (macOS)** | Keychain svc `"sidecar-quota"`, account = 이메일 |
| **백업 cred (Linux)** | `$HOME/.sidecar/quota:quota/creds/<email>.json` · `chmod 600` |

## 캐시 동작 (45초 TTL)

```
첫 /quota:quota (활성 계정)     →  endpoint 호출  →  캐시 write  →  표시
...
| 시기 | 위치 | 역할 |
|---|---|---|
| 이전 | dancinlab/wilson `plugins/quota:quota/main.hexa` (917줄) | full multi-account + cred-store + switch |
| 0.1.0 | sidecar `skills/quota:quota/bin/_quota.hexa` | view-only port; wilson harness 결합 제거 |
| 0.2.0 | (동일) | + `/quota:quota add` 레지스트리 쓰기 (metadata-only) |
| 0.3.0 | (동일) | + cred-store + `/quota:quota switch` + `remove` + `refresh` (wilson 등가) |
| 0.4.0 | (동일) | + 계정별 닉네임 (옵션) · `nick` 동사 · 모든 ref가 nick/email/index 통과 |
| 0.4.1 | `commands/<verb>.md` × 8 (picker 실험) | 서브커맨드 개별 picker 노출 |
| **현재 (0.7.0)** | `commands/quota:quota.md` 단일 디스패처 | `/quota:quota <verb>` (bare → status) · `autoadd` verb (quota-autoadd hook 연동) |
