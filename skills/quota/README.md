...
각 서브커맨드가 개별 picker 항목으로 노출. `/quota` (네임스페이스 landing, bare → status)도 유지.

| 동사 | picker 진입 | 결과 |
|---|---|---|
| (bare) | `/quota` 또는 `/quota:quota` | 기본 → status |
...
| **현재 (0.4.1)** | + `commands/<verb>.md` × 8 | 각 서브커맨드가 picker에 개별 항목으로 노출 — `/quota:status` 등 |
