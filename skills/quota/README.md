...
## 0.4.0 — view + 레지스트리 + cred-store + switch + **닉네임**

| 동사 | 결과 |
|---|---|
| `/quota` | 기본 → status |
| `/quota status` | 활성 계정 + 라이브 5h/7d (45초 캐시). 닉네임 등록되어 있으면 `nick (email)` 형태 |
| `/quota list` | 등록된 계정 목록. 닉네임 있는 행만 닉네임 표시 |
| `/quota add [<nick>]` | 현재 계정 등록 + 자격증명 백업 캡처. 닉네임은 **옵션** (생략 가능) |
| `/quota nick <ref> [<nick>]` | 기존 계정의 닉네임 설정/해제. 빈 인자 = 해제 (aliases: `name`, `rename`) |
...
| **현재 (0.4.0)** | (동일) | + 계정별 닉네임 (옵션, 기본값=이메일만) · `nick` 동사 · 모든 ref가 nick/email/index 통과 |
