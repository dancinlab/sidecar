...
@D quota := "Claude 5h/7d limits + multi-account registry + cred swap + nicks" :: skill
  ...
  dont = "treat a `(unavailable)` row as the limit being unlimited · hammer /quota faster than the 45s cache · cite limits without showing whether they came from live or cache · route OTHER users' traffic through these creds (ToS red-line — not a multi-tenant router) · `/quota switch` to an account that was never `add`-ed while logged in (no creds to swap in) · pick a digit-only nickname (e.g. `1` collides with numeric index lookup)"
