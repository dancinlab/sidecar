#!/usr/bin/env python3
# research :: YouTube transcript extractor.
#
...
def main():
    ...
    if not args:
        print("research/yt: usage: /research:yt "
              "<youtube-url-or-id> [lang]")
        return
    vid = video_id(args[0])
    if not vid:
        print("research/yt: could not parse a video id from %r"
              % args[0])
        return
    ...
    try:
        ...
        if err:
            print("research/yt: %s" % err)
            return
        track = pick_track(tracks, lang)
        text = transcript_text(track)
    except urllib.error.URLError as e:
        print("research/yt: network error — %s" % e)
        return
    except (ValueError, ET.ParseError) as e:
        print("research/yt: failed to parse the transcript — %s" % e)
        return
