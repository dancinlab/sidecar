...
2. 각 프론티어를 **research 라운드(r1)** 로 발사 — 기본은 **싼 research Agent**(web/arxiv·문헌·코드 census · GPU 0 · mini-safe · `run_in_background:true`). 비싼 implement 는 research 가 정당화한 뒤에만(§2 gate). 프롬프트 자기완결(미시도 레버 탐색 + 인용 + "다음 라운드 명명 + depletion 테스트").
3. 🔬 랩 report(§4) 출력.
...
## 7. 동시성
고정 레인 cap 없음 — 머신 부하 정직하게. research 라운드는 동시 다발 OK(싸다). implement 라운드는 worktree 격리 + pool 호스트 직렬화 고려(같은 GPU 연타 회피).
