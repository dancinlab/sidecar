...
3. **Deepen the hot families.** Dispatch one subagent per hot family (in parallel — all
   in a single message via the Agent tool), each given the target plus that family's
   `gap` lenses, returning a concrete finding + a suggested fix per gap. Use the
   `Explore` agent for read-only investigation, `general-purpose` when a fix needs
...
Skip triage. Dispatch **8 subagents in parallel** (one per family, all in a single
message), each sweeping the target through all of its lenses (5 for F1·F2·F3·F5·F7·F8,
6 for F4 and F6 — `occams-razor` lives in both) and returning per-lens findings. Use
this when triage might miss a latent gap and an exhaustive audit is worth the cost.
