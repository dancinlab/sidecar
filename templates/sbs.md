...
위 `resolved:` 줄(`resolved: mode=.. axis=.. weights=.. source=…`)을 **권위로** 따른다 — resolver 가
CODE 로 `~/.sidecar/recommend-default` 를 읽어 우선순위를 계산한 값이므로 모델이 다시 해석하지 않는다.
`mode:` 한 줄은 그대로 echo 해 모드를 선언한다. 모델은 `~/.sidecar/recommend-default` 를 따로 읽지 않는다.
...
2. **한 번에 하나씩, 채팅으로 (NOT AskUserQuestion).** 비자명 문항은 easy 7요소로:
   ...
   **7요소 골격 = `sidecar easy` 결정적 빌트인 래핑 (손으로 짜지 말 것)** — `easy` 주입 스타일이 쓰는 바로 그 SSOT backbone:
   ```
